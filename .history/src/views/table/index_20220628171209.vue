<template>
  ...
  width="400"
  trigger="click">
  <el-checkbox v-for="item in tableTitle" :key="item" v-model="item.show" :label="item.label" size="large" />
  <el-button slot="reference">筛选列</el-button>
</el-popover>
     
      <!-- <el-select class="filter-item" style="margin-left: 10px;width: 90px" placeholder="筛选列">
        <el-option v-for="item in tableTitle" :key="item" v-model="checkboxVal">
    ...
    >
      ...
      <el-table-column v-for="item in tableTitle" :key="item" :label="item.label" :prop="item.value" :sortable="item.sortable" align="center" :width="item.with" />
...
export default {
  ...
  watch: {
    checkboxVal(valArr) {
      console.log(valArr)
      this.tableTitle = this.tableTitle.filter(i => valArr.indexOf(i) >= 0)
