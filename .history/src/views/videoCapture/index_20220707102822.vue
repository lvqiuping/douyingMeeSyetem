    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 600px; margin-left:50px;">
        ...
        <el-form-item label="发布时间" prop="publicTime">
          ...
          <el-radio v-model="temp.publicTime" label="1">15天内</el-radio>
          <el-radio v-model="temp.publicTime" label="2">一个月内</el-radio>
          <el-radio v-model="temp.publicTime" label="3">半年内</el-radio>
        </el-form-item>
        <el-form-item label="消耗点数上限" prop="test4">
          <el-input-number v-model="temp.test4" :min="0" :max="10" @change="handleChange1" /><span>点</span>
          <div style="font-size: 12px; color: #ccc">（填0则不限制点数）</div>
        </el-form-item>
        <el-form-item label="定时监控频率" prop="test5">
          <span>每</span><el-input-number v-model="temp.test5" :min="0" @change="handleChange2" /><span>天运行一次</span>
          <div style="font-size: 12px; color: #ccc">（默认为0，不开启自动监控）</div>
        </el-form-item>
        <el-form-item label="最长监控天数" prop="test6">
          <span>持续监控</span><el-input-number v-model="temp.test6" :min="0" @change="handleChang3" /><span>天后停止</span>
          <div style="font-size: 12px; color: #ccc">（默认为0，不开启自动监控）</div>
        </el-form-item>
