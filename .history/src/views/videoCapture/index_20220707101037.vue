    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 600px; margin-left:50px;">
        ...
        <el-form-item label="定时监控频率" prop="type">
          <span>每</span><el-input-number v-model="num" :min="1" :max="10" @change="handleChange" /><span>天运行一次</span>
          <div style="font-size: 12px; color: #ccc">（默认为0，不开启自动监控）</div>
        </el-form-item>
        <el-form-item label="消耗点数上限" prop="type">
          <span>持续监控</span><el-input-number v-model="num" :min="1" :max="10" @change="handleChange" /><span>天后停止</span>
          <div style="font-size: 12px; color: #ccc">（默认为0，不开启自动监控）</div>
        </el-form-item>
